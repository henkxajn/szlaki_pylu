// Szlaki Pyłu — Web v2.3 (stabilny render)
// Naprawa: rysowanie nie zależy od update() (render wywoływany jawnie).
// Dodatkowo: biomy mają kolor jako liczba 0xRRGGBB (bez konwersji Phaser).
// Jeśli coś się wywali, błąd dopisze się do Logu.
...
function makePlanetEconomy(biome){
  ...
  if (biome.id==="MARS"){ stock.set(0,stock.get(0)+8); stock.set(6,Math.max(0,stock.get(6)-3)); demand.set(6,demand.get(6)+3); }
  if (biome.id==="ICE"){ stock.set(6,stock.get(6)+8); demand.set(0,demand.get(0)+2); }
  if (biome.id==="SATURN"){ stock.set(1,stock.get(1)+6); stock.set(5,stock.get(5)+3); demand.set(2,demand.get(2)+2); }
  if (biome.id==="OCEAN"){ stock.set(2,stock.get(2)+7); demand.set(1,demand.get(1)+2); }
  if (biome.id==="JUNGLE"){ stock.set(2,stock.get(2)+6); demand.set(4,demand.get(4)+2); }
  if (biome.id==="BARREN"){ stock.set(3,stock.get(3)+6); stock.set(0,stock.get(0)+4); }

  return {stock,demand,shock,installs};
}
function ensurePlanetEconomy(planet){
  if (!planet.econ && planet.biome) planet.econ = makePlanetEconomy(planet.biome);
}
...
function generateWorld(){
  ...
  while (placed<planetCount && attempts<8000){
    attempts++;
    ...
    if (neighbors(q,r).some(n => world.hexes.get(key(n.q,n.r))?.type===HexType.PLANET)) continue;
...
function descForHex(h){
  ...
  return "Handluj zasobami i kupuj instalacje (produkcja zasila magazyn planety).";
}
function showEvent(h){
  ...
  const onPlanet = h.type===HexType.PLANET;
  ui.btnMarket.style.display = onPlanet ? "inline-block":"none";
  ui.btnInstall.style.display = onPlanet ? "inline-block":"none";
  ui.btnAction.style.display = onPlanet ? "inline-block":"none";
  ui.planetTags.innerHTML = onPlanet ? h.biome.tags.map(t=>`<span class="pill">${t}</span>`).join(" ") : "";
}
...
function renderInspector(h){
  ...
  if (h.type===HexType.PLANET){
    ...
    const prices = Goods.map(g=>({g, p:price(h.econ,g.id)})).sort((a,b)=>b.p-a.p);
    meta.push(`Biome: ${h.biome.name}`);
    meta.push(`Drogie tu: ${prices.slice(0,3).map(x=>x.g.name).join(", ")}`);
    meta.push(`Instalacje: ${h.econ.installs.length ? h.econ.installs.map(i=>`${i.id} lvl ${i.level}`).join(" • ") : "brak"}`);
...
function drawInspectorArt(h){
  const c=ui.planetCanvas, ctx=c.getContext("2d");
  ...
  for (let i=0;i<18;i++){
    const x=(i*37)%w, y=(i*53)%hh;
    ctx.beginPath(); ctx.arc(x,y,(i%3===0)?1.3:0.9,0,Math.PI*2); ctx.fill();
  }
  ...
  const col = intToCss(h.biome.color);
  ctx.beginPath(); ctx.fillStyle=col; ctx.arc(cx,cy,26,0,Math.PI*2); ctx.fill();
...
function showInstallations(planet){
  ...
  ui.installDesc.textContent = "Co turę produkują towary do magazynu planety.";
  ...
  for (const inst of Installations){
    ...
    btn.onclick=()=>{
      ...
      refreshResources(); log(`Instalacja: ${inst.name} → poziom ${(found?.level) ?? 1}.`);
      showInstallations(planet); renderInspector(planet);
    };
...
ui.btnInstall.onclick = ()=>{ const h=world.current; if (h?.type===HexType.PLANET) showInstallations(h); else log("Instalacje tylko na planetach."); };
...
function requestRender(){ needsRender = true; if (gameScene) gameScene.time.delayedCall(0, ()=>renderWorld()); }

function renderWorld(){
  if (!gameGfx || !gameScene) return;
  ...
  for (let i=0;i<140;i++){
    const sx = (i*131) % w;
    const sy = (i*271) % h;
    g.fillCircle(sx, sy, (i%3===0)?1.2:0.8);
  }
...
class MainScene extends Phaser.Scene {
  ...
  create(){
    ...
    // render przy resize
    this.scale.on("resize", ()=>requestRender());
  }
}

function drawHex(g, cx, cy, size, fillColor, fillAlpha, lineColor, lineAlpha){
  const pts=[];
  for (let i=0;i<6;i++){
...
function drawHexOutline(g, cx, cy, size, lineColor, lineAlpha, width){
  const pts=[];
  for (let i=0;i<6;i++){
    const ang = Phaser.Math.DegToRad(60*i - 30);
    pts.push({x:cx+Math.cos(ang)*size, y:cy+Math.sin(ang)*size});
